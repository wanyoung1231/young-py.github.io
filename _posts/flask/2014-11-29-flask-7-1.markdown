...
与大多数其他web框架不同，Flask不施加特定的大型项目组织；应用程序的结构完全交给开发人员。在这一章，一个可能的方式来组织一个大型应用程序包和模块。这种结构将用于书中其余的例子。
...
The blueprint is registered with the application inside the create app() factory function, as shown in Example 7-5.

blueprint是注册到应用程序内创建应用程序()工厂函数,如示例7 - 5所示。
